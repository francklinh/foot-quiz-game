...
import { ApiChallengesService, ApiChallenge } from '../services/api-challenges.service';

interface ApiChallengesListProps {
  userId: string;
  className?: string;
}

export function ApiChallengesList({ 
  userId, 
  className = '' 
}: ApiChallengesListProps) {
  const [challenges, setChallenges] = useState<ApiChallenge[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);
  const [activeTab, setActiveTab] = useState<'all' | 'pending' | 'active' | 'completed'>('all');

  const challengesService = new ApiChallengesService();
  ...
  const loadChallenges = async () => {
    try {
      setLoading(true);
      setError(null);
      const challengesList = await challengesService.getChallenges(userId);
